...
NavigationGrid::NavigationGrid(const std::vector<std::string> grid) {
	nodeSize = stoi(grid[0]);
	gridWidth = stoi(grid[1]);
	gridHeight = stoi(grid[2]);
	startX = stoi(grid[3]);
	startY = stoi(grid[4]);
	...
	for (int y = 0; y < gridHeight; ++y) {
		for (int x = 0; x < gridWidth; ++x) {
			...
			char type = grid[y + 5][x];
			n.type = type;
			n.position = Vector3((float)((x * nodeSize) + startX), 0, (float)((y * nodeSize) + startY));
		}
	}
...
bool NavigationGrid::FindPath(const Vector3& from, const Vector3& to, NavigationPath& outPath) {
	...
	int fromX = ((int)(from.x-startX) / nodeSize);
	int fromZ = ((int)(from.z-startY) / nodeSize);

	int toX = ((int)(to.x-startX) / nodeSize);
	int toZ = ((int)(to.z-startY) / nodeSize);
