...
void GameWorld::AddPlayer(GameObject* o) {
	if (player) {
		RemoveGameObject(player, true);
	}
	player = o;
	AddGameObject(o);
}

void GameWorld::ClearPlayer() {
	player = nullptr;
}

void GameWorld::AddGameObject(GameObject* o) {
	gameObjects.emplace_back(o);
	if (o->GetName() == "Goal") goals++;
...
void GameWorld::UpdateWorld(float dt, bool updateObjects) {
	...
	if (updateObjects) {
		for (int i = 0; i < gameObjects.size(); i++) {
			...
			gameObjects[i]->UpdateObject(dt, *this);
			if (size > gameObjects.size()) i -= (size - gameObjects.size());
		}
	}
