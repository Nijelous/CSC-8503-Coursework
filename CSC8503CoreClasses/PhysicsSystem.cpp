...
void PhysicsSystem::Update(float dt) {
	...
	if (gravityFirstFrame) gravityFirstFrame = false;
}
...
void PhysicsSystem::UpdateCollisionList() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = allCollisions.begin(); i != allCollisions.end(); ) {
		if (i->a->GetWorldID() == -572662307 || i->b->GetWorldID() == -572662307) {
...
void PhysicsSystem::BasicCollisionDetection() {
	...
	for (auto i = first; i != last; i++) {
		...
		if (gravityFirstFrame) (*i)->GetPhysicsObject()->Awaken();
		for (auto j = i + 1; j != last; j++) {
			if ((*j)->GetPhysicsObject() == nullptr || 
				((*i)->GetPhysicsObject()->IsSleeping() && (*j)->GetPhysicsObject()->IsSleeping()) ||
				((*i)->GetPhysicsObject()->GetInverseMass() == 0 && (*j)->GetPhysicsObject()->GetInverseMass() == 0)) continue;
		...
		if (!(*i)->GetPhysicsObject()->IsSleeping() && (!hasCollided || (*i)->GetPhysicsObject()->GetLinearVelocity() < Vector3(0.1f, 0.1f, 0.1f))) {
 			(*i)->GetPhysicsObject()->Sleep(applyGravity, gravity, (*i)->GetTransform().GetPosition());
		}
		(*i)->GetPhysicsObject()->SetColliding(hasCollided);
	}
}
...
void PhysicsSystem::BroadPhase() {
	...
	for (auto i = first; i != last; i++) {
		Vector3 halfSizes;
		if (!(*i)->GetBroadphaseAABB(halfSizes) || (*i)->GetPhysicsObject() == nullptr) continue;
		Vector3 pos = (*i)->GetTransform().GetPosition();
		tree.Insert(*i, pos, halfSizes);
		if (gravityFirstFrame) (*i)->GetPhysicsObject()->Awaken();
		(*i)->GetPhysicsObject()->SetColliding(false);
	}
	tree.OperateOnContents([&](std::list<QuadTreeEntry<GameObject*>>& data) {
		...
		for (auto i = data.begin(); i != data.end(); i++) {
			for (auto j = std::next(i); j != data.end(); j++) {
				...
				if ((info.a->GetPhysicsObject()->IsSleeping() || info.a->GetPhysicsObject()->GetInverseMass() == 0) &&
					(info.b->GetPhysicsObject()->IsSleeping() || info.b->GetPhysicsObject()->GetInverseMass() == 0)) continue;
...
void PhysicsSystem::NarrowPhase() {
	for (std::set<CollisionDetection::CollisionInfo>::iterator i = broadphaseCollisions.begin(); i != broadphaseCollisions.end(); i++) {
		...
		if (CollisionDetection::ObjectIntersection(info.a, info.b, info)) {
			...
			ImpulseResolveCollision(*info.a, *info.b, info.point);
			allCollisions.insert(info);
			info.a->GetPhysicsObject()->SetColliding(true);
			info.b->GetPhysicsObject()->SetColliding(true);
		}
	}
	std::vector<GameObject*>::const_iterator first;
	std::vector<GameObject*>::const_iterator last;
	gameWorld.GetObjectIterators(first, last);
	for (auto i = first; i != last; i++) {
		if (!(*i)->GetPhysicsObject()->IsSleeping() && 
			(!(*i)->GetPhysicsObject()->IsColliding() || (*i)->GetPhysicsObject()->GetLinearVelocity() < Vector3(0.1f, 0.1f, 0.1f))) {
			(*i)->GetPhysicsObject()->Sleep(applyGravity, gravity, (*i)->GetTransform().GetPosition());
		}
	}
}
...
void PhysicsSystem::IntegrateAccel(float dt) {
	...
	for (auto i = first; i != last; i++) {
		...
		if (applyGravity && inverseMass > 0 && !object->IsSleeping()) accel += gravity;
...
void PhysicsSystem::IntegrateVelocity(float dt) {
	...
	for (auto i = first; i != last; i++) {
		...
		if (linearVel.AbsSum() > 1.0f) object->Awaken();
		if (object->IsSleeping() && !object->IsColliding()) position = object->GetSleepPos();
		else {
