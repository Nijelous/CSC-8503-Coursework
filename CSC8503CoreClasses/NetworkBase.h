...
struct DataRequestPacket : public GamePacket {
	char stringData[256];

	DataRequestPacket(const std::string message) {
		type = BasicNetworkMessages::Data_Request_Message;
		size = (short)message.length();

		memcpy(stringData, message.data(), size);
	}
	int GetRequestFromData() {
		return (int)stringData[0] - 48;
	}
	std::string GetExtraData() {
		std::string realString(stringData);
		realString.resize(size);
		return realString.substr(1);
	}
};
