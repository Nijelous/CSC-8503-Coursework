...
void TestBehaviourTree() {
	...
	BehaviourAction* lookForTreasure = new BehaviourAction("Look For Treasure", [&](float dt, BehaviourState state)->BehaviourState {
		if (state == Initialise) {
			...
			bool found = rand() % 2;
			if (found) {
				std::cout << "I found treasure\n";
				return Success;
			}
			std::cout << "No treasure lies here...\n";
			return Failure;
		}
		return state;
		});
...
class PauseScreen : public PushdownState {
	...
	void OnAwake() override {
		std::cout << "Press U to unpause game, or Escape to go back to main menu!\n";
	}
};
...
int main() {
	...
	while (w->UpdateWindow() && running) {
		...
		if (Window::GetKeyboard()->KeyPressed(KeyCodes::NUMPAD7)) {
			w->SetWindowPosition(0, 0);
		}
		if (Window::GetKeyboard()->KeyPressed(KeyCodes::NUMPAD9)) {
			w->SetWindowPosition(500, 0);
		}
		if (Window::GetKeyboard()->KeyPressed(KeyCodes::NUMPAD1)) {
			w->SetWindowPosition(0, 300);
		}
		if (Window::GetKeyboard()->KeyPressed(KeyCodes::NUMPAD3)) {
			w->SetWindowPosition(500, 300);
		}
